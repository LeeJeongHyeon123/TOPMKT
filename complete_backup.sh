...
git tag -a "$TAG_NAME" -m "완전 백업 포인트 - React 전환 전 ($BACKUP_DATE)"
echo -e "${GREEN}✅ Git 태그 생성: $TAG_NAME${NC}"

# 9. Git 저장소 백업
echo -e "${BLUE}📦 Git 저장소 백업 중...${NC}"
git clone --bare "$PROJECT_DIR" "$BACKUP_DIR/git_repo_$BACKUP_DATE.git"
...
=== 데이터베이스 정보 ===
$(mysql -u root -pDnlszkem1! -e "SELECT VERSION() as 'MySQL Version', NOW() as 'Backup Time';")

=== 테이블 요약 ===
$(cat "$BACKUP_DIR/table_statistics_$BACKUP_DATE.txt")
...
3. Git 롤백: git checkout $TAG_NAME
...
echo -e "${YELLOW}🏷️ Git 태그:${NC} $TAG_NAME"
...
echo "   백업 가이드: /workspace/docs/BACKUP_완전_백업_가이드.md"
